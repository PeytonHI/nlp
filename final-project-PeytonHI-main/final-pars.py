...
def is_similar_query(model, claim, content):
    ...
    for par in content:
        sent_tensor = model.encode(par, convert_to_tensor=True)
        ...
        similarity_index[par] = similarity
...
def getTopParagraphs(pars, k=3):
    pars_dict_list = []
    for pars_score_dict in pars:
        pars_dict = sorted(pars_score_dict, key=pars_score_dict.get, reverse=True)[:k]
        pars_dict_list.append(pars_dict)

    return pars_dict_list
...
def main():
    ...
    for data in tqdm(datamini):
        ...
        for data_point in wiki_search_titles[:5]:
            ...
            if evidence_doc is None:
                break
            # sentence:score
            par_scores = is_similar_query(model, claim, evidence_doc) # check similarity between claim and docs
            if par_scores is None:
                break
            
            similar_list.append(par_scores)

        top_k_pars = getTopParagraphs(similar_list) # select top 3 articles with highest cosine sim scores



        response = generate_label(claim, top_k_pars) 
    ...
    with open("custom-multiQPara.jsonl", "w") as fout:
