...
def generate_label(claim):
    response = ollama.chat(
        model="llama3.2:1b",
        messages=[
            {
                "role": "user",
                "content": f"You are a fact checker. Determine if the following claim is true: \nClaim: {claim}\nOnly reply with SUPPORTS, REFUTES, or NOT ENOUGH INFO and nothing else. Choose the label with the strongest evidence, even if it refutes the claim."
            }
        ]
    )
    return response
...
def main():
    ...
    with open("custom-multiQ-nodocs.jsonl", "w") as fout:
